...
public class MutableHashTable2<BT, PT> implements MemorySegmentSource {

	private static final Logger LOG = LoggerFactory.getLogger(MutableHashTable2.class);
	...
	private PT currentProbeRecord;

	private MutableHashTable.ProbeIterator<PT> probeIterator;
	...
	private boolean probeMatchingPhase = true;
	...
	public MutableHashTable2(TypeSerializer<BT> buildSideSerializer, TypeSerializer<PT> probeSideSerializer,
	...
	public MutableHashTable2(TypeSerializer<BT> buildSideSerializer, TypeSerializer<PT> probeSideSerializer,
			...
			IOManager ioManager, boolean useBloomFilters)
	{
		this(buildSideSerializer, probeSideSerializer, buildSideComparator, probeSideComparator, comparator,
	...
	public void open(boolean buildOuterJoin) {
		...
		this.bucketIterator = new HashBucketIterator<>(
	...
	public void addBuildRecord(BT record) throws IOException {
		final int hashCode = hash(buildSideComparator.hash(record), 0);
		insertIntoTable(record, hashCode);
	}

	public void buildFinish() throws IOException {
	...
	public boolean probe(PT probeRecord) throws IOException {
		Preconditions.checkState(this.probeMatchingPhase, "Bug: probe should only happens in probe matching phase");

		this.currentProbeRecord = probeRecord;
		final int hash = hash(probeSideComparator.hash(probeRecord), this.currentRecursionDepth);
		...
		if (p.isInMemory()) {
			this.recordComparator.setReference(probeRecord);
			...
			if (status == BUCKET_STATUS_IN_FILTER) {
				...
				if (this.bloomFilter.testHash(hash)) {
					p.insertIntoProbeBuffer(probeRecord);
				}
			} else {
				p.insertIntoProbeBuffer(probeRecord);
			}
	...
	public void probeFinish() throws IOException {
		probeMatchingPhase = false;
	}

	public boolean moreRecord() throws IOException {
	...
	public PT getCurrentProbeRecord() {
		if (this.probeMatchingPhase) {
			return this.currentProbeRecord;
		} else {
			return null;
		}
	}
	...
	private boolean processProbeIter() throws IOException {
		...
		PT next;
		while ((next = this.probeIterator.next()) != null) {
			if (probe(next)) {
				return true;
			}
		}
	...
	private boolean processUnmatchedBuildIter() throws IOException  {
		if (this.unmatchedBuildVisited) {
			return false;
		}

		this.probeMatchingPhase = false;
	...
	private boolean prepareNextPartition() throws IOException {
		...
		this.probeIterator = new MutableHashTable.ProbeIterator<>(probeReader, this.probeSideSerializer.createInstance());
	...
	protected void buildTableFromSpilledPartition(final HashPartition<BT, PT> p) throws IOException {
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(String.format("totalNumBuffers[%d], numWriteBehindBuffers[%d], " +
							"availableMemory[%d], writeBehindBuffersAvailable[%d]",
					this.totalNumBuffers, this.numWriteBehindBuffers, this.availableMemory.size(),
