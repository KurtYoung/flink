...
public class BatchBarrierHandler implements CheckpointBarrierHandler {
	...
	private static final Logger LOG = LoggerFactory.getLogger(BatchBarrierHandler.class);
	...
	private InputGate inputGate1, inputGate2;

	// ------------------------------------------------------------------------

	public BatchBarrierHandler(InputGate inputGate) {
		this(null, null);
	}

	public BatchBarrierHandler(InputGate inputGate1, InputGate inputGate2) {
		this.inputGate1 = inputGate1;
		this.inputGate2 = inputGate2;
		this.inputGate = InputGateUtil.createInputGate(new InputGate[]{inputGate1, inputGate2});
	...
	public BufferOrEvent getNextNonBlocked(InputSelection inputSelection) throws Exception {
		while (true) {
			InputGate inputGateToRead = inputGate;
			if (InputSelection.FIRST.equals(inputSelection)) {
				inputGateToRead = inputGate1;
			} else if (InputSelection.SECOND.equals(inputSelection)) {
				inputGateToRead = inputGate2;
			}
			BufferOrEvent next = inputGateToRead.getNextBufferOrEvent();
